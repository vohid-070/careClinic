...
const { externalEmployees } = storeToRefs(monitoringStore);
const { externalEmployeeDetail } = storeToRefs(monitoringStore);
...
const getEmployee = async () => {
  ...
  dataEmployee.value = await monitoringStore.getMonitoringExternalEmployees(
    currentDate,
    search.value,
    currentPage.value
  );
...
const getEmployeeDetail = async (id) => {
  ...
  employeeDetailData.value =
    await monitoringStore.getMonitoringExternalEmployeeById(
      currentDate,
      employeeId.value,
  ...
  <div class="grid">
    <div class="col-12">
      <div class="card my-4 p-2">
        <DataTable
          :value="externalEmployees.content"
        ...
        >
          ...
          <Column field="id" header="Хамкор доктор ID" />
          <Column field="fullName" header="Хамкор доктор ФИО" />
          ...
          <template #footer>
            <Paginator
              ...
              :rows="1"
              :total-records="externalEmployees.totalPages"
            >
              <template #start></template>
              <template #end>
                <div class="totalElements">
                  <b>Элементлар сони: </b> {{ externalEmployees.totalElements }}
                </div>
              </template>
            </Paginator>
          </template>
        ...
        >
          ...
          <DataTable
            ...
            :value="externalEmployeeDetail.content"
          >
            <template #empty>Ҳеч қандай маълумот топилмади</template>
          ...
          <template #footer>
            <Paginator
              ...
              :rows="1"
              :total-records="externalEmployeeDetail.totalPages"
            >
              <template #start></template>
              <template #end>
                <div class="totalElements">
                  <b>Элементлар сони: </b>
                  {{ externalEmployeeDetail.totalElements }}
                </div>
              </template>
            </Paginator>
          </template>
