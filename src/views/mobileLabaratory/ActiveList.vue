...
import { useMobileStore } from "@/stores/mobileLabaratory";
import { storeToRefs } from "pinia";
import { GoogleMap, Marker } from "vue3-google-map";
import { useToast } from "primevue/usetoast";

const MobileStore = useMobileStore();
const { mobilLaboratory } = storeToRefs(MobileStore);
const { comentById } = storeToRefs(MobileStore);
...
const getActiveList = async () => {
  ...
  await MobileStore.MobileActive(search.value, currentPage.value);
...
const getComments = async (id) => {
  ...
  commentId.value = id;
  dataComment.value = await MobileStore.GetCommentById(id);
...
const AddComment = async () => {
  ...
  const data = await MobileStore.AddComment(commentId.value, comment);
  if (data.status == 200) {
...
const Serviced = async () => {
  ...
  if (comentById.value.name) {
    const data = await MobileStore.Serviced(commentId.value, comment);
    if (data.status == 200) {
      showComments.value = false;
...
<template>
  <div>
    <DataTable :value="mobilLaboratory.content" :loading="tableLoading">
      <template #empty>Ҳеч қандай маълумот топилмади</template>
      <template #header>
        <InputText
          v-model="search"
          placeholder="Қидириш"
          @input="getActiveList(search)"
        />
      </template>
    ...
    <Paginator
      ...
      v-on:update:first="getActiveList"
      :rows="1"
      :total-records="mobilLaboratory.totalPages"
    >
      <template #start> </template>
      <template #end>
        <div class="totalElements">
          <b>Элементлар сони: </b> {{ mobilLaboratory.totalElements }}
        </div>
      </template>
    </Paginator>
